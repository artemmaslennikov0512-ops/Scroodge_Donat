...
import { getStreamerByDonateLinkId } from "@/lib/streamer";

type PageProps = { params: Promise<{ token: string }> };

/** Страница доната по уникальной ссылке /d/{token}. Один токен — один стример, нельзя перепутать аккаунты. */
export default async function DonateByLinkPage({ params }: PageProps) {
  const { token } = await params;
  const donateLinkId = token?.trim();
  if (!donateLinkId) notFound();

  const streamer = await getStreamerByDonateLinkId(donateLinkId);
  ...
  const displayUsername = user?.username ?? streamer.slug;
